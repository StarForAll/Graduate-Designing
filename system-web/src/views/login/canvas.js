...
export const lonWave = () => {
  let starlings = function (n, r, t, o, e, u, i, f) {
    let a = f.onSetup;
    void 0 === a && (a = null);
    let v = f.onRepeat;
    void 0 === v && (v = null);
  ...
  let initialize = function initialize (vertices) {
...
export const canvasParticle = (function () {
  ...
  function canvasInit (canvasConfig) {
    ...
    body.onmousemove = function (e) { let event = e || window.event; canvas.mouse = { x: event.clientX, y: event.clientY }; };
    document.onmouseleave = function () { canvas.mouse = undefined; };
    setInterval(function () { drawPoint(canvas); }, 40);
  }
  function canvasSize (canvas) {
    let width = document.getElementById('canvasView').style.width;
    let height = document.getElementById('canvasView').style.height;
  ...
  function drawPoint (canvas) {
    ...
    let point;
    let dist;
    context.clearRect(0, 0, canvas.element.width, canvas.element.height); context.beginPath(); context.fillStyle = 'rgb(' + canvas.config.color + ')'; for (let i = 0, len = canvas.config.count; i < len; i++) {
  ...
  function borderPoint (point, canvas) {
    let p = point; if (point.x <= 0 || point.x >= canvas.element.width) { p.vx = -p.vx; p.x += p.vx; } else if (point.y <= 0 || point.y >= canvas.element.height) { p.vy = -p.vy; p.y += p.vy; } else { p = { x: p.x + p.vx, y: p.y + p.vy, vx: p.vx, vy: p.vy }; }
    return p;
  }
  function drawLine (context, canvas, mouse) {
    let dist;
    context = context || canvas.context; for (let i = 0, len = canvas.config.count; i < len; i++) {
      canvas.points[i].max_conn = 0; for (let j = 0; j < len; j++) {
        if (i != j) {
