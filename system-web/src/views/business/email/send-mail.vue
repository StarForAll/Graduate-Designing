  ...
  <div>
    <Card class="warp-card" dis-hover>
      <Form :label-width="80" :model="form" :rules="ruleInline">
        ...
        <FormItem>
          <Button :loading="isShowSaveButtonLoading" @click="addOrUpdateEmail" type="primary"  v-privilege="'email-send'">保存</Button>&nbsp;&nbsp;
          <Button :loading="isShowSaveButtonLoading" @click="sendEmail" type="primary"  v-privilege="'email-send'">发送</Button>
        </FormItem>
      </Form>
    </Card>
  </div>
      ...
      try {
        ...
        if(this.form.sendStatus==0){
          ...
          if(this.$route.query.id){
            res = await emailApi.updateEmail(this.form);
          }else{
            res = await emailApi.addEmail(this.form);
          }
        ...
        this.goBack();
          ...
          this.$Spin.hide();
          this.$Message.success('该邮件已经发送，不能重复发送');
          this.goBack();
        }
      } catch (error) {
        //TODO zhuoda sentry
