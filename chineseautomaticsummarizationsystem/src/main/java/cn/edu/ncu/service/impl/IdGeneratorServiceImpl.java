...
import cn.edu.ncu.service.IdGeneratorService;
...
public class IdGeneratorServiceImpl implements IdGeneratorService {
    ...
    void init() {
        ...
        List<IdGenerator> idGeneratorEntities = idGeneratorMapper.selectAll();
    ...
    private IdGeneratorPOJO validateParams(IdGeneratorEnum idGeneratorEnum, int stepLength) {
        if (stepLength < 1) {
            throw new BusinessException(BaseDataExceptionCode.ID_GENERATOR_TOO_SHORT_CODE,
                    new Exception("IdGenerator， step过短" + stepLength));
        }

        IdGeneratorPOJO idGeneratorPOJO = this.idGeneratorMap.get(idGeneratorEnum.getId());
        if (idGeneratorPOJO == null) {
            throw new BusinessException(BaseDataExceptionCode.ID_GENERATOR_NOT_FOUND_CODE,
                    new Exception("IdGenerator， id 不存在" + idGeneratorEnum));
        }
